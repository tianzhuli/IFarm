...
public class FarmControlSystemService {
	...
	public JSONArray farmControlRegion(Integer farmId) {
		JSONArray array = JSONArray.fromObject(farmControlSystemDao.farmControlRegion(farmId));
		return array;
	}
	...
	public void produceWFMControlTaskCommand(WFMControlTask controlTask) {
		...
		for (int i = 0; i < controlDevicesArrayList.size(); i++) {
			...
			for (int j = 0; j < arrayList.size(); j++) {
				...
				if ("forward".equals((String) objects[2])) {
					forwardbits[(int) objects[1]] = 1;
				} else if ("reverse".equals((String) objects[2])) {
					reversebits[(int) objects[1]] = 1;
				}
			...
			command.setReversebits(reversebits);
