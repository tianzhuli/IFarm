...
public class ControlStrategyUtil {
	...
	public static void topRollerShuttersControl(ControlTask controlTask, List<Object[]> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if ("rise".equals(functionCode)) {
				riseBit = (int) farmControlTerminal[3];
			} else if ("fall".equals(functionCode)) {
				fallBit = (int) farmControlTerminal[3];
			}
		}
		if ("rise".equals(controlOperation)) {
			bits[riseBit] = 1;
		} else if ("fall".equals(controlOperation)) {
			bits[fallBit] = 1;
		}
	...
	public static void sideRollerShuttersControl(ControlTask controlTask, List<Object[]> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			Object[] farmControlTerminal = list.get(i);
			String functionCode = (String) farmControlTerminal[5];
			if ("stretch".equals(functionCode)) {
				stretchBit = (int) farmControlTerminal[3];
			} else if ("shrink".equals(functionCode)) {
				shrinkBit = (int) farmControlTerminal[3];
			}
		}
		if ("stretch".equals(controlOperation)) {
			bits[stretchBit] = 1;
		} else if ("shrink".equals(controlOperation)) {
			bits[shrinkBit] = 1;
		}
		controlTask.setControlTerminalbits(bits);
	}
	...
	public static void valveControl(ControlTask controlTask, List<Object[]> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			Object[] farmControlTerminal = list.get(i);
			String functionCode = (String) farmControlTerminal[5];
			if ("forward".equals(functionCode)) {
				forwardBit = (int) farmControlTerminal[3];
			} else if ("reverse".equals(functionCode)) {
				reverseBit = (int) farmControlTerminal[3];
			}
		}
		if ("forward".equals(controlOperation)) {
			bits[forwardBit] = 1;
		} else if ("reverse".equals(controlOperation)) {
			bits[reverseBit] = 1;
		}
		controlTask.setControlTerminalbits(bits);
